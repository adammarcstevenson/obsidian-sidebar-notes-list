...
import type { OpenType, Timestamp } from '../types'
...
export class SidebarNotesListSettingTab extends PluginSettingTab {
  ...
  public display(): void {
    ...
    // Open type
    new Setting(containerEl)
      .setName(l10n('settingOpenTypeLabel'))
      .setDesc(l10n('settingOpenTypeDescription'))
      .addDropdown((dropdown) => {
        const options: Record<OpenType, string> = {
          'tab': l10n('settingOpenTypeOptionTab') as string,
          'split': l10n('settingOpenTypeOptionSplit') as string,
          'window': l10n('settingOpenTypeOptionWindow') as string
        }
        dropdown
          .addOptions(options)
          .setValue(state.settings.openType)
          .onChange(value => {
            state.settings.setOpenType(value as OpenType)
          })
      })
